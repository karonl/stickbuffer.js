...
module.exports = class StickBuffer extends EventEmitter {
  ...
  static toBuffer(string, encoding) {
    const bufBody = Buffer.from(string, encoding || 'utf8') // Payload 7B 22 6D 65 73 73 61 67....
    const bufHead = Buffer.allocUnsafe(3)
    bufHead.writeUInt8(0xdf) // Flag DF
    bufHead.writeUInt16BE(bufBody.length, 1) // Length 03 0A
    return Buffer.concat([bufHead, bufBody], bufHead.length + bufBody.length);
  }
